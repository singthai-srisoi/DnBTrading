import { pgTable, integer, bigint, varchar, timestamp, date, text, boolean, doublePrecision, smallint, index, foreignKey, primaryKey, unique, check } from "drizzle-orm/pg-core"
...
export const authGroupPermissions = pgTable("auth_group_permissions", {
	...
	index("auth_group_permissions_group_id_b120cbf9").using("btree", table.groupId.asc().nullsLast()),
	index("auth_group_permissions_permission_id_84c5c92e").using("btree", table.permissionId.asc().nullsLast()),
...
export const authPermission = pgTable("auth_permission", {
	...
	index("auth_permission_content_type_id_2f476e4b").using("btree", table.contentTypeId.asc().nullsLast()),
...
export const authUserUserPermissions = pgTable("auth_user_user_permissions", {
	...
	index("auth_user_user_permissions_permission_id_1fbb5f2c").using("btree", table.permissionId.asc().nullsLast()),
	index("auth_user_user_permissions_user_id_a95ead1b").using("btree", table.userId.asc().nullsLast()),
...
export const djangoAdminLog = pgTable("django_admin_log", {
	...
	index("django_admin_log_content_type_id_c4bce8eb").using("btree", table.contentTypeId.asc().nullsLast()),
...
export const inventoriesInventory = pgTable("inventories_inventory", {
	...
	index("inventories_inventory_customer_id_6e232f4f").using("btree", table.customerId.asc().nullsLast()),
	index("inventories_inventory_driver_id_440b3e60").using("btree", table.driverId.asc().nullsLast()),
	index("inventories_inventory_product_id_8199cb3d").using("btree", table.productId.asc().nullsLast()),
	index("inventories_inventory_supplier_id_c1910ce3").using("btree", table.supplierId.asc().nullsLast()),
	index("inventories_inventory_ticket_no_4343be3a_like").using("btree", table.ticketNo.asc().nullsLast().op("varchar_pattern_ops")),
	index("inventories_inventory_vehicle_id_8686ba5c").using("btree", table.vehicleId.asc().nullsLast()),
...
export const tokenBlacklistBlacklistedtoken = pgTable("token_blacklist_blacklistedtoken", {
	...
	tokenId: bigint("token_id", { mode: 'number' }).notNull().references(() => tokenBlacklistOutstandingtoken.id),
}, (table) => [
...
export const tokenBlacklistOutstandingtoken = pgTable("token_blacklist_outstandingtoken", {
	...
	index("token_blacklist_outstandingtoken_user_id_83bc629a").using("btree", table.userId.asc().nullsLast()),
